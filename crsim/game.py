...
class PlayerState:
    ...
    _leaked_elixir: float = 0.0  # cumulative elixir wasted to the cap (leak)
...
class CRGame:
    ...
    def _regen_elixir(self) -> None:
        ...
        for ps in self.players:
            new = ps.elixir + rate
            if new > MAX_ELIXIR:
                ps._leaked_elixir += new - MAX_ELIXIR  # measure waste before clip
            ps.elixir = min(new, MAX_ELIXIR)
    ...
    def _process_combat(self, entity: Entity) -> None:
        ...
        # Clear the target so the unit retargets instead of locking up forever.
        ...
        if entity.attack_timer <= 0:
            ...
            if entity.has_projectile and entity.projectile_speed > 0:
                ...
                if entity.is_splash and entity.splash_radius > 0:
                    # Melee splash (e.g. Valkyrie) is centred on the ATTACKER;
                    # the primary target is always hit. Splash victims also
                    # receive the attacker's stun / inferno reset.
                    for other in self.entities:
                        if (
                            other.alive
                            ...
                            and (
                                other is target
                                or other.distance_to(entity) <= entity.splash_radius
                            )
                        ):
                            other.take_damage(self._crown_adjusted_entity(entity, dmg, other))
                            if stuns and stun_dur > 0:
                                other.apply_stun(stun_dur)
                            if resets and other.inferno_dps_max > 0:
                                other.inferno_ramp_time = 0.0
                else:
    ...
    def _apply_evo_on_attack(self, entity: Entity, target: Entity) -> None:
        ...
        # Skeletons "Skele-Cloning": each attack spawns a Skeleton up to max.
        if evo.spawn_on_attack and entity.card_type == CardType.SKELETONS:
            living = sum(
                1
                for e in self.entities
                if e.alive and e.owner == entity.owner
                and e.card_type == CardType.SKELETONS
            )
            cap = evo.max_spawns or 8
            if living < cap and len(self.entities) < MAX_ENTITIES:
                sk_def = CARD_DEFS[CardType.SKELETONS]
                clone = entity_from_card(
                    eid=self._alloc_eid(),
                    owner=entity.owner,
                    card_def=sk_def,
                    x=entity.x + 0.5,
        ...
        if (
            ...
            if dist > 1e-6:
                target.x += (dx / dist) * evo.knockback_tiles
                target.y += (dy / dist) * evo.knockback_tiles
                target.x = max(0.0, min(float(ARENA_W - 1), target.x))
                target.y = max(0.0, min(float(ARENA_H - 1), target.y))
        ...
        for proj in self.projectiles:
            ...
            if dist <= travel:
                ...
                if proj['is_splash'] and proj['splash_radius'] > 0:
                    # Ranged splash is centred on the impact point (target).
                    # All victims also take the stun / inferno reset.
                    for other in self.entities:
                        if (
                            other.alive
                            ...
                            and (
                                other is target
                                or other.distance_to(target) <= proj['splash_radius']
                            )
                        ):
                            other.take_damage(self._crown_pct_adjust(other, dmg, pct))
                            if proj['stuns'] and proj['stun_duration'] > 0:
                                other.apply_stun(proj['stun_duration'])
                            if proj['resets_inferno'] and other.inferno_dps_max > 0:
                                other.inferno_ramp_time = 0.0
                else:
    ...
    def shaping_signal(self, player: int) -> dict:
        """Read-only board scalars for potential-based reward shaping.

        No side effects. ``tower_diff`` and ``elx_diff`` are symmetric in
        [-1, 1]; ``leak`` is this player's cumulative wasted elixir.
        """
        opp = 1 - player

        def side_hp(p: int) -> tuple[float, float]:
            tot = mx = 0.0
            kt = self.king_towers[p]
            if kt is not None:
                tot += max(kt.hp, 0.0)
                mx += kt.max_hp
            for pt in self.princess_towers[p]:
                tot += max(pt.hp, 0.0)
                mx += pt.max_hp
            return tot, mx

        my_hp, my_mx = side_hp(player)
        op_hp, op_mx = side_hp(opp)
        tower_diff = (my_hp / my_mx - op_hp / op_mx) if my_mx and op_mx else 0.0
        elx_diff = (self.players[player].elixir - self.players[opp].elixir) / MAX_ELIXIR
        leak = self.players[player]._leaked_elixir
        return {"tower_diff": tower_diff, "elx_diff": elx_diff, "leak": leak}

    def potential(self, player: int) -> float:
        """Phi(s, player) for potential-based shaping (policy-invariant).

        Phi = W_TOWER * tower_diff + W_ELIXIR * elx_diff. The shaped per-step
        reward GAMMA^dt * Phi(s') - Phi(s) leaves the optimal policy unchanged.
        The leak penalty is applied separately by the trainer.
        """
        sig = self.shaping_signal(player)
        return W_TOWER * sig["tower_diff"] + W_ELIXIR * sig["elx_diff"]
