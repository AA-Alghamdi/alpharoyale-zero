...
def evaluate_vs_random(
    ...
    """Quick evaluation: play n games vs an opponent, return win rate.

    ``opponent`` is an optional scripted agent (e.g. ProAgent) exposing
    ``select_action(game, player)``. If None, the opponent plays random moves.
    Use a STRONG opponent (ProAgent) as the curriculum benchmark — win-rate vs
    random saturates at ~100% and gives no learning gradient.
    ...
    for _ in range(n_games):
        ...
        while not game.done:
            ...
            # Player 1: scripted opponent if provided, else random
            if opponent is not None:
                p1_action_id = opponent.select_action(game, 1)
            else:
...
def main() -> None:
    ...
    parser.add_argument("--init-checkpoint", type=str, default=None,
                        help="Load model weights (e.g. a BC checkpoint) before training")
    ...
    if getattr(args, "init_checkpoint", None):
        ...
        logger.info("Loaded init checkpoint %s (missing=%d unexpected=%d)",
                    args.init_checkpoint, len(missing), len(unexpected))
...
def _run_single_gpu(
    ...
    worker = SelfPlayWorkerV2(
        ...
        league=league,  # wire PFSP opponent sampling (was never passed)
    )
    ...
    while trainer.step_count < args.max_steps:
        ...
        if steps_since_eval >= args.eval_interval:
            ...
            # The meaningful benchmark: win-rate vs the strong teacher (ProAgent).
            # vs-random saturates at ~100% and gives no curriculum gradient.
            ...
            logger.info(
                "Eval @ step %d: %.1f%% vs ProAgent | %.1f%% vs random",
                trainer.step_count, win_rate * 100, wr_random * 100,
            )
    ...
    logger.info(
        "Training complete! Final: %.1f%% vs ProAgent | %.1f%% vs random",
        final_pro * 100, final_wr * 100,
    )
