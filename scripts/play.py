#!/usr/bin/env python3
"""Interactive play: watch a policy play or play against it (text mode).

Usage:
    # Watch policy vs policy
    python scripts/play.py --checkpoint checkpoints/best.pt --mode watch

    # Play against the policy (you control player 0)
...
def interactive_game(model_path: str, device: torch.device) -> None:
    """Play interactively against the policy."""
    model = load_model(model_path, device)
    mcts_cfg = MCTSConfig(n_simulations=200, temperature=0.1)
    policy_player = MCTSPlayer(model=model, config=mcts_cfg, device=device)

    game = CRGame(seed=42)

    print("=== ClashRoyale-Zero Interactive Mode ===")
    print("You are Player 0 (bottom). Policy is Player 1 (top).")
    ...
    while not game.done:
        ...
        # Opponent turn
        policy_action_id, _ = policy_player.select_action(game, 1, deterministic=True)
        policy_action = _action_id_to_action(policy_action_id, 1)

        game.step([human_action, policy_action])
...
def watch_game(model_path: str, device: torch.device, delay: float = 0.2) -> None:
    """Watch the policy play against itself."""
    model = load_model(model_path, device)
