...
def test_pro_emits_legal_actions():
    assert _agent_emits_legal_actions(ProAgent())


def test_pro_dominates_wait_agent():
    a_wins, b_wins, draws = play_match(ProAgent(), WaitAgent(), n_games=8, seed=0)
    assert a_wins == 8, (a_wins, b_wins, draws)


def test_pro_is_stronger_than_meta():
    # ProAgent is the top scripted rung: counter-aware defense + value-gated
    # spells should out-score the plain Meta bot head-to-head. Deterministic
    # (no RNG), so this is a stable inequality.
    tot_a = tot_b = tot_d = 0
    for rseed in (0, 100, 200):
        a, b, d = play_match(ProAgent(), MetaAgent(), n_games=10, seed=rseed)
        tot_a += a
        tot_b += b
        tot_d += d
    winrate = tot_a / (tot_a + tot_b + tot_d)
    assert winrate > 0.6, (tot_a, tot_b, tot_d)
