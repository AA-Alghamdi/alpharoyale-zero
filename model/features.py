...
#   [23]     is_evolved (this unit deployed evolved)
#   [24]     is_hero (hero unit)
#   [25]     is_champion (champion unit)
#   [26]     shield_hp / 2000 (if shielded)
#   [27]     freeze_timer / 4.0 (frozen status)
#   [28]     rage_timer / 7.5 (raged status)
#   [29]     poison_timer / 8.0 (poisoned)
#   [30]     next_hit_is_charge (charged attack pending)
#   [31]     stun_timer / 2.0
#   [32]     load_progress / load_time (pre-attack readiness)
#   [33]     champion ability ready (off cooldown)
#   [34]     champion ability cooldown fraction remaining
#   [35]     invisible (Archer Queen cloak active)
#   [36]     attack-speed boost active
#   [37]     damage-reduction active (Monk protection)
#   [38]     evo one-shot shield active (Knight)
#   [39]     reserved / zero-padded
...
def extract_entity_features(
    ...
    for i in range(n):
        ...
        # Shield status (no max stored; normalize against a typical shield pool)
        f[26] = min(e.shield_hp / 2000.0, 1.0) if e.has_shield else 0.0
        ...
        # Charge: the next hit is a charged (bonus-damage) attack
        f[30] = 1.0 if e.next_hit_is_charge else 0.0

        # Stun timer
        f[31] = min(e.stun_timer / 2.0, 1.0)

        # Load progress (pre-attack readiness)
        f[32] = e.load_progress / e.load_time if e.load_time > 0 else 0.0

        # Champion-ability state so the policy can learn to time activations.
        if e.is_champion:
            f[33] = 1.0 if e.ability_cooldown_timer <= 0 else 0.0  # ability ready
            if e.ability_cooldown > 0:
                f[34] = min(e.ability_cooldown_timer / e.ability_cooldown, 1.0)
        # Active ability effects (apply to champions and evolved units alike).
        f[35] = 1.0 if e.invisible_timer > 0 else 0.0  # Archer Queen cloak
        f[36] = 1.0 if e.attack_speed_timer > 0 else 0.0  # attack-speed boost
        f[37] = 1.0 if e.damage_reduction_timer > 0 else 0.0  # Monk protection
        f[38] = 1.0 if e.evo_shield else 0.0  # Knight-evolution shield
